...
public interface ShowcaseRepository extends JpaRepository<Showcase, Long> {
	...
	@Query("select s from Showcase s join fetch s.member m join fetch m.image where s.id < :showcaseId")
	Slice<Showcase> findAllByIdLessThan(long showcaseId, Pageable pageable);

	@Query("select s from Showcase s join fetch s.member m join fetch m.image where s.category = :category and s.id < :showcaseId")
	Slice<Showcase> findAllByCategoryAndIdLessThan(Category category, long showcaseId, Pageable pageable);

	@Query("select s from Showcase s join fetch s.member m join fetch m.image where s.content like concat('%', :query, '%') and s.id < :showcaseId")
